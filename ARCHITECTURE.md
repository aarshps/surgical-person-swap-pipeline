...
```mermaid
graph TD
    A[Source Images] -->|Embedding Learning| B(Identity Profile: aya.npy)
...
- **`AyaOrchestrator`**: The central controller. Responsible for learning Aya's identity from reference images, calculating the 3D-profile, and managing the surgical integration pipeline.
...
1.  **Identity Learning**: The `learn_aya` method calculates the average facial embedding from reference photos in `data/references/aya/` and saves it as a persistent NumPy array.
2.  **Surgical Swap**:
...
3.  **Output**: Refined images are exported to `output/samples/aya_swaps/`.
